...
// Static globals
typedef std::map<std::string, std::vector<double>> EphemerisDatabase;
...
void JplApproximateEphemeris::VLoad()
{
   ...
   g_ephemerisDatabase.reset(new EphemerisDatabase());
   EphemerisDatabase& data = *g_ephemerisDatabase;
...
{
   EphemerisDatabase::const_iterator it = g_ephemerisDatabase->find(name);
   return (it != g_ephemerisDatabase->end());
}
...
void JplApproximateEphemeris::VQueryDatabase(const std::string& name, const Epoch& epoch, OrbitalElements& orbitalElements)
{
    ...
    const auto& data = g_ephemerisDatabase->at(name);
